        ...
        if(status !== undefined)
            return(
                <div className={"ventilator-status " + isActiveClass}>
                    <svg xmlns="http://www.w3.org/2000/svg" className={"profile-img svg-"+status.toLowerCase() + (this.props.ventilator.status === 'alarm' ? " alarm-fill" : "")} width="16" height="16" viewBox="0 0 32 32">
                    ...
                    <div className="info">
                        <p>{name}</p>
                        <p className="status-data"><span className={"text-"+status.toLowerCase() + (this.props.ventilator.status === 'alarm' ? " alarm-text" : "")}>{status.toUpperCase()}</span></p>
                    </div>
                </div>
            );
        else
